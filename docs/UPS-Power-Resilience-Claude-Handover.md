...
- [x] Add the utility host to Beszel and expose only required read-only UPS metrics.
  ...
  dashboard. Re-verified 2026-08-30: `beszel-agent` is active/running on
  `nut-server` and has remained up since the host's 2026-08-29 reboot.
  This required a new OPNsense firewall rule permitting
...
- [x] Add actionable power/NUT checks and alerts to HomeLab Doctor/reporting.
  ...
  units (`proxmox-ups`, `nas-ups`, and `network-ups`) detected via `upsc -l`;
  each unit's `ups.status` contains `OL` (fails if not — e.g. on battery or
  communication lost); and warns if `battery.charge` drops below 50%.
  Re-verified with the complete HomeLab Doctor run on 2026-08-30: the NUT
