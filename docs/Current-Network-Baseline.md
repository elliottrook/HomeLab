...
Arista DCS-7050TX-64-R
  ...
  +-- Et23 Synology-NIC-A
  +-- Et33 UniFi-PoE-Uplink (remote Port 9; native VLAN 10, tagged VLANs 30,40)
  +-- Et40 OPNsense-LAN (trunk; native VLAN 10, tagged VLANs 30,40)
...
The interface descriptions above were applied and verified on the Arista, with the final Et3 and Et8 assignments completed on 2026-08-08. Stale Synology descriptions were removed from Et20 and Et21.
...
- The temporary switch has been removed. Proxmox is directly connected on Arista Et3 and the Mac Studio is directly connected on Et8; both are access ports in VLAN 10 with PortFast enabled.
...
- Arista Et40 and Et33 are operating as trunks with native VLAN 10 and allowed VLANs 10,30,40. Temporary test ports Et1 and Et2 are access ports in VLANs 40 and 30 respectively.
...
- Homepage includes network, virtualization, storage, media, media-automation, application-management, external-service and SSH-access groups. The external-service tiles link to the Tailscale administration console and GitHub. SSH tiles launch Proxmox, Docker LXC, OPNsense and TrueNAS connections through the client device's registered SSH application. The dashboard and its internal management targets remain intended for LAN or Tailscale access only.
...
| 20 | Servers | 192.168.20.0/24 | Proxmox, TrueNAS, Synology and hosted services |
...
- Arista: VLAN 40 named `Guest`; Et40 trunk native VLAN 10 with VLANs 10,40 allowed; Et1 temporary access test port.
...
- Arista: VLAN 30 named `IoT`; Et40 and Et33 are trunks with native VLAN 10 and VLANs 10,30,40 allowed; Et2 is the temporary access test port. The validated running configuration was saved to startup-config.
