...
- Prometheus 3.13.2 LTS and Grafana OSS 13.2.0 run in unprivileged LXC 109 at `192.168.20.31`. Prometheus has a 90-day/20 GB local retention ceiling and collects bounded, read-only metrics from Proxmox, TrueNAS, Frigate and all three NUT-managed UPS units. Grafana is private on TCP 3000 and its first provisioned HomeLab overview dashboard is online; alerting remains disabled during the dashboard pilot. Cross-VLAN access is limited to collector-to-Proxmox TCP 8006 and collector-to-NUT TCP 3493. Frigate TCP 5000 and TrueNAS Graphite TCP 9109 accept only their documented collector/source peers; exporter listeners otherwise remain on loopback.
