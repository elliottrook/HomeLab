...
- **Proxmox guest archives** — TrueNAS's native `rsynctask` (SSH, the existing restricted `homelab-backup` account, forced `rrsync` rooted at `/mnt/backups/dump`) pulls to `/mnt/Media/backup/homelab-proxmox-guests`. Scope is VMIDs 100–109, 111 (NetBox), 113 (Aster Wiki), 114 (news aggregator) and LXC 115 (Paperless-ngx). LXC 115 was added on 2026-09-21; manual job 52077 copied all six retained archives with matching filenames/sizes. Its newest archive matched the Proxmox SHA-256 and passed Zstandard integrity verification on both hosts. An isolated restore from the TrueNAS copy into temporary LXC 915 passed SQLite integrity, container health and login HTTP checks; the temporary guest was removed. Offsite scope was corrected on 2026-09-23: whole-LXC 115 archives are excluded from the relay, and all seven previously uploaded archive versions were removed. Only the reviewed service reconstruction export is stored offsite; database, documents, OCR, summaries, runtime state and credentials stay local/TrueNAS. Post-deployment backup and isolated restore evidence is in the Paperless runbook. Daily at 04:00. VMID 113 was added on 2026-09-12; its first copied archive matched the Proxmox source SHA-256 and passed complete Zstandard verification on both hosts.
...
the current TrueNAS backup hub's Proxmox-leg scope, 100–109/111/113). Isolated
...
| OpenBao LXC 117 | M1 validates sealed/unsealed state and protected audit writes; permanent Doctor integration follows with the broker milestone | Daily all-guests Proxmox coverage plus an authenticated Raft snapshot at `backups:dump/openbao-app/openbao-m1-synthetic.snap`; encrypted 2-of-3 human recovery shares are held separately | The Raft snapshot was restored into network-isolated disposable LXC 118, unsealed with the original human-held shares, and verified for synthetic data, audit, broker policy and AppRole metadata before LXC 118 was destroyed |
...
| Aster Wiki LXC 113 | Doctor checks intake health, daily collector state, local archive age and the monthly corpus-health report | Current LXC archive retained locally, checksum-mirrored to TrueNAS and included in the encrypted off-site relay; the two private Git repositories also remain independently bundle-recoverable | Clean repository bundles plus protected retained originals rebuilt a verified human corpus and byte-identical mirror in an isolated LXC104 temporary path; production paths were untouched |
