...
[`docs/reference/Aster-Operations.md`](../../reference/Aster-Operations.md).
...
[`docs/reference/AI-Hermes-Second-Brain.md`](../../reference/AI-Hermes-Second-Brain.md).
...
| 2026-09-21 | Runtime/driver update and xe-reset monitoring | Live-diagnosed a reported "PCIe x1" restriction as the documented Arc/`xe` cosmetic reporting artifact (real link unchanged at PCIe 3.0 x8, confirmed via the on-card switch's upstream bridge). Found one contained `xe` engine reset in dmesg (2026-09-19 15:32:42, `engine_class=ccs`, no host freeze, not correlated with an active aster-llama OOM); upstream `intel/compute-runtime#842` remains open. Staged llama.cpp b11081 beside the production b10507 binary and benchmarked both with `llama-bench` against the same `UD-IQ4_XS` model/flags (pp4096 86.2→76.4 t/s, tg128 6.83→8.55 t/s); promoted b11081 to `aster-llama.service` with the old binary and unit file kept as rollback — live production decode rose from a historical ~4.8-5.0 tok/s to ~5.7-5.8 tok/s. Then trialed `mesa-vulkan-drivers` 26.1.2 from `trixie-backports` (pinned `NotAutomatic`, one package plus two stock-repo dependencies) on LXC 110 and re-benchmarked b11081: pp4096 76.4→193.3 t/s, tg128 8.55→11.85 t/s, no new dmesg errors; kept. Final live production decode measured 11.82 tok/s on an identical prompt, more than double the original baseline. Declined the SYCL/Level-Zero and vLLM-XPU pilots: both depend on Level-Zero, which this host's small physical BAR already proved (2026-08-30, above) refuses to initialize, and the upstream SYCL `qwen35`/B60 correctness bug is also still open. Added HomeLab Doctor check `check_xe_reset`, watching the last 72h of kernel log for a recurrence of the engine-reset class. | llama.cpp and Mesa updates kept, verified stable with no GPU resets during or after; SYCL/vLLM pilots correctly gated on the still-unresolved BAR limitation, not re-attempted; new Doctor coverage ran live and correctly surfaced the known Sept 19 event as a warning, expected to self-clear after 72h with no recurrence |
