...
> Status: Graduated — all milestone completion gates passed
>
> Project owner: Jason
>
> Proposed: 2026-09-10
...
**Current milestone:** Complete — Milestones 1–5 are graduated.
...
`1.2.0` and Aster snapshot integration are live. Three real unattended collector
runs, a scheduled changed-source acceptance and deliberate failure preservation
pass. The accepted changed content is
`4640e34f0afde03449713c2ddd07fefe9ddf01ff91f5a2fe06a3c6085ef2e6f9`;
the next-cycle input is safely restored to the retained original. Milestone 3 is
graduated. Milestone 4's
...
**Next safe action:** normal operation through the active daily collector and
monthly corpus-health timers. Future source enrollment remains an explicit
owner action through the authenticated portal.
...
- [x] **HomeLab Doctor:** add bounded service, pipeline age/result, quarantine
  count and accepted-snapshot checks with synthetic failure tests.
- [x] **Monitoring/alerting:** add non-duplicative availability/run-age metrics
  and owner-visible alerts after the service target is selected.
- [x] **Backup and recovery:** protect both Git repositories, collector state,
  permitted originals and deployed snapshot; prove isolated restore.
- [x] **NetBox:** use existing asset/service IDs read-only where applicable; add
  a service record only if NetBox's adopted authority requires one.
- [x] **Human wiki:** this project creates it; operator guidance and recovery
  links are graduation requirements.
- [x] **Aster mirror/snapshot:** create the separately recoverable derived
  mirror and extend the validated manifest/retrieval path without changing its
  authority.
- [x] **Operational reference/runbooks:** link or version-import selected
  material; current-state authority stays in `homelab-reference`.
- [x] **Repository documentation:** update architecture, addressing,
...
- [x] **Homepage/service discovery:** add a private operator link with a useful
...
- [x] **Automation and schedules:** add observable daily and monthly jobs with
  locking, missed-run behavior and retained last-success state.
- [x] **Security inventory:** document source-local secrets, restrictive modes,
...
- [x] Schedule a daily no-change-safe run and publish a bounded health report
...
- [x] Implement changed-document analysis using the fixed mirror schema.
- [x] Preserve page/section locators and source hashes for every salient claim.
- [x] Add verification that each claim is supported by its cited source and
  that uncertainty, version scope and destructive warnings survive reduction.
- [x] Create asset, service, dependency, symptom and recovery indexes.
- [x] Prevent duplication and mark superseded entries without deleting their
  provenance history.
- [x] Produce a deterministic validated snapshot consumable by Aster's current
...
- [x] Add both repositories, collector state, accepted originals where legally
  permitted and the deployed mirror snapshot to verified local, off-host and
  encrypted off-site coverage.
- [x] Rebuild both the human presentation and mirror from clean repositories
  plus protected originals in an isolated environment.
- [x] Test rollback after a deliberately bad upstream update and a deliberately
  bad generated summary.
- [x] Document adding equipment, adding a source, reviewing quarantine,
...
| 2026-09-10 | 2 authentication and schedule gate | Jason completed the real owner password/passkey flow at `https://wiki.elliottrook.com` and confirmed return to “Add a private knowledge source.” After confirmation, enabled the reviewed persistent daily timer (`03:35 America/Vancouver`, randomized by at most 15 minutes); systemd reports it enabled/active with its first trigger at 10:46 UTC. The first Doctor run exposed that dashboard SQLite reads created coordination files under the intake identity, making collector status read-only. Switched dashboard/history connections to immutable read mode, added a regression that they create no WAL/SHM files, repaired the empty live coordination-file ownership and redeployed. Collector status then returned durable state, dashboard remained healthy, and a full Doctor run passed the Aster Wiki check. Its remaining failures are pre-existing Arista link and Backup Synology findings outside this project | Authentication, observable scheduling and Doctor integration are operational. Milestone 2 remains open only for the two required daily-cycle observations, including the deliberate failure preservation proof |
| 2026-09-10 | 3 deterministic mirror candidate | Began Milestone 3 while awaiting daily-cycle evidence. Added a no-network deterministic extractive generator that accepts only hash-verified human-corpus inputs, emits fixed-schema derived entries with exact line locators, builds asset/service/symptom/dependency/provenance indexes, and records accepted-input and generation manifests. The verifier requires every mirrored claim excerpt to exist at its cited locator under the accepted source hash. CLI build mode performs two isolated builds and rejects any hash mismatch before producing output. Extended publication with claim deduplication, explicit supersession when a changed claim occupies a prior locator, pre-publication verification, atomic replacement, retained last-good output and rollback. Three tests prove byte-identical packaging, rejection of an unsupported/tampered entry, and changed-entry supersession plus rollback; the full suite passes 33/33 | The source-located deterministic packaging, supersession and rollback core is locally proven. PDF extraction, warning/contradiction semantics and deployed Aster integration remain open |
| 2026-09-10 | 3 Aster snapshot integration candidate | Extended the existing deterministic Aster snapshot builder with an optional validated mirror root. It maps only verified mirror Markdown into `mirror/`, forces `derived-memory` authority, preserves the complete human-source path, exact locator and source hash in `.aster-provenance.json`, rejects secret/provenance/count mismatches, and records the accepted mirror hash as repository state. Aster retrieval now returns `human_source` and `source_locator`; its system policy requires derived labeling and human-source fallback whenever mirror evidence is missing, uncertain, conflicting or insufficient. Two isolated builder tests pass. The local Mac lacks Aster's FastAPI dependency, so the complete agent regression suite is deferred to its existing deployed virtual environment before activation | The bounded snapshot/retrieval integration is locally ready without widening Aster's tools or network access. Deployment and live regression remain gated; PDF extraction and semantic adversarial coverage remain open |
| 2026-09-10 | 3 PDF and reduction-safety candidate | Added bounded PDF text extraction through `pdftotext` with strict UTF-8 decoding, a 5 MiB extracted-text ceiling and page-plus-line source locators. PDF and text reductions share the same unsafe-content rejection and exact-excerpt verifier. Added deterministic warning, uncertainty, version-scope and contradiction indexes without rewriting source claims. Tests prove page-located PDF claims, fail-closed handling of unsafe extracted PDF text and semantic preservation; the complete wiki suite passes 35/35 | Milestone 3's local reduction pipeline now covers accepted text and PDF inputs with explicit semantic-risk indexes. Live dependency installation, deployment, clean double-build evidence and Aster regression/retrieval validation remain open |
| 2026-09-10 | 3 live mirror generation | Installed Debian's `poppler-utils` on LXC113 and deployed the rollback-backed mirror CLI at local commit `14b9497`. Two independent live invocations each accepted and verified three exact-source claims with identical accepted-input hash `63202faa197a4b9b25df932e91bbc7606a619958ae2c183d6d14979fdbce966c` and content hash `3b85478a2c0ee72a824f6c71c8747bc3b1f4631d06e4e1adb4d6159888a5c182`; a last-good mirror remains present. The collector timer remained active. The verified live mirror was streamed read-only to the build host, where two clean Aster snapshot builds each contained 30 sources and compared byte-for-byte identical (aggregate file hash `698d6f7d55bfb0ac65df3849351b7ad995176fcd7cb54c050327590c1b43f468`) | Deterministic live mirror generation and clean snapshot consumption pass. Aster-side regression, deployment and retrieval/fallback evidence remain before the Milestone 3 gate |
| 2026-09-10 | 3 Aster retrieval correction | The first Aster activation passed 62/62 staged regressions and health but failed functional acceptance because unrelated high-authority results displaced the mirror from the model's two-result context. It was rolled back immediately. A strong-match mirror ranking correction then exposed the entry and exact route, but a second model check revealed that front matter crowded the exact claim out of the excerpt; that activation was also rolled back. The final correction anchors derived excerpts at `## Source-located claim`. It passed 62/62 staged tests, health, default retrieval and the model-level check: Aster answered that `TEST-42` means the synthetic battery fixture is absent, labeled the evidence `derived-memory`, linked `docs/upstream/synthetic-ups-manual/content.txt` at lines 7–9 and warned that the fixture is not deployed hardware | Fail-closed activation and rollback worked twice, and the corrected Aster retrieval/presentation path passed exact-claim and provenance acceptance |
| 2026-09-10 | 3 final changed-document pipeline | Added verified reuse of unchanged source entries, retained changed-locator supersession, and an explicit recovery index; bumped the output pipeline to `1.2.0`. The wiki suite passes 36/36. On LXC113, both the migration build and independent unchanged-input build verified three entries with the same input hash `63202faa197a4b9b25df932e91bbc7606a619958ae2c183d6d14979fdbce966c` and content hash `f07ded1b60f10c98f581834bac104b0a763c2f6d2e99911240167ba638fbe096`; recovery index, last-good output and active collector timer were confirmed. Two clean final Aster snapshot builds contained 30 sources, were byte-identical and had archive hash `72a660fb0bb21263c1a2c06b9ac19f1f9528454313631eaad3b7d73ec93a02dd` | The final mirror artifact is deterministic, incrementally generated, verified and ready for the separate Aster snapshot activation |
| 2026-09-11 | 3 graduation | Activated the final pipeline `1.2.0` snapshot on LXC104 after archive-hash, 30-source, three-mirror-entry, authority, human-route, locator and restrictive-permission validation. The service restarted healthy. Direct retrieval returned the exact claim that `TEST-42` means the synthetic battery fixture is absent, with `derived-memory`, `docs/upstream/synthetic-ups-manual/content.txt` and lines 7–9; the fallback policy assertion passed. The final model-level request repeated the exact meaning, authority, complete human route and synthetic/non-deployed qualification | Milestone 3 completion gate passed. Same-version live builds and clean snapshots are deterministic, all tested claims remain exact-source-located, and Aster consumes the final mirror without promoting it to authority |
| 2026-09-12 | 2 unattended daily cycles | Systemd journal and timer state show true scheduled runs at 2026-09-11 10:46:38 UTC and 2026-09-12 10:46:37 UTC. Both completed without operator action; the second accepted one source with zero failures/quarantines, and immediate corpus verification checked one accepted source successfully. The timer remains enabled and active with its next trigger scheduled for 2026-09-13 10:46 UTC | The required two unattended daily observations pass. Milestone 2 remains open only for the controlled deliberate-failure proof that the last accepted corpus is preserved |
...
| 2026-09-13 | 2 graduation and project completion | Systemd triggered the staged changed-source run at 10:42:48 UTC as invocation `0766435ecfdb41ed93af6e5e40cd1674`, producing run `run-1a75e19dec214a20`: one accepted source, zero unchanged, failures or quarantines. The accepted lock names normalized and protected-original SHA-256 `4640e34f0afde03449713c2ddd07fefe9ddf01ff91f5a2fe06a3c6085ef2e6f9`; the accepted document has the same hash and corpus verification passes 1/1. Restored only the retained `5f0c7a6a159cee7e4daf1b30d222464686ce6e4e06581efbaa27403d4050c0df` original as the next-cycle upload, preserving `aster-collector:aster-wiki` mode `0640`; the accepted changed revision and lock remain intact and verify clean. Intake, daily collector and monthly corpus-health units remain active, with the next daily trigger scheduled for 2026-09-14 | Milestone 2 completion gate passed through three true unattended cycles, including changed-source acceptance and separately proven fail-closed preservation. With Milestones 1–5 complete, the whole project is graduated without widening Aster's authority |
