...
| 2026-08-30 | Open issue logged | Record-stream segment fragmentation on `front_of_house` (main/record path only, detect path unaffected) — Frigate's own bandwidth anomaly detector flagged ~17x normal bandwidth; survived a full `docker compose down`/`up`; camera's own encoder settings checked and look reasonable. Root cause unconfirmed. Full write-up: [07-Surveillance.md](../07-Surveillance.md#known-issues). | Open |
