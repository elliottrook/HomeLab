...
1. Recheck AI-PAM owner activity and five live hashes. Abort on new source/runtime drift, M6 deployment in progress, unexpired pending/approved requests, in-flight operations or uncertainty. Confirm the already deployed M6 write gateway and unit command line match the re-baseline; abort on further change. Do not race another task.
...
5. Install the two files root:root 0644. Run schema migration once as `hlabroker` with both daemons stopped. Verify DB integrity and preserved terminal-state counts. Unexpired pending/approved authorizations are an abort condition from step 1; expired legacy rows remain unchanged and unusable; never backfill a policy hash.
...
## Subsequent acceptance work and current gate
