...
| 2026-09-07 | — | Manual review of tonight's 7 duplicate + 6 scatter candidates, with Jason. File-size/format cross-check (not just duration match) caught 2 more real issues before any delete: "Uncaged" folder held 2 internal self-duplicate files (not unique tracks, safe); "Willie Nelson/Django And Jimmie" (MP3 192kbps) vs "Compilations/Django And Jimmie" (AAC ~257kbps) — evidence contradicted the default keeper heuristic, held back rather than guessed | 6 of 7 duplicate pairs deleted (lesser copy) after full manifest + size/format verification; 1 held back pending a real listen/decision; stray KT Tunstall M4A duplicate track deleted. Also found and fixed 2 more detector bugs: multi-disc detection missed this library's "...[Disc N]" suffix-on-album-name convention (false-flagged Luke Bryan's real 2-disc set as scatter); "ambiguous" duplicate comparisons were silently dropped from every report (now surfaced as `ambiguous_for_reference`, revealing the *Still Crazy After All These Years* case is correctly never auto-touched, plus a few more real near-duplicates — e.g. Paul Simon *Graceland*, Carrie Underwood *Blown Away* — worth Jason's attention later, not tonight's scope) | Claude |
| 2026-09-07 | — | Django and Jimmie resolved: no audio-listening capability available, so used tag/encoder-provenance evidence instead — both copies traced to the same original purchase (identical MusicBrainz/ISRC/Acoustid/iTunes-normalization tags), but the AAC copy carried iTunes's own `Encoding Params` atom (written only when iTunes itself performs the encode), while the MP3 could not be ruled out as a downstream transcode of it. Jason approved acting on that evidence over the original default keeper heuristic | Moved the 14 AAC tracks from `Compilations/Django And Jimmie` into `Willie Nelson/Django And Jimmie`, replacing the lower-provenance MP3s; removed 2 further stray folders found during cleanup (one held a byte-identical duplicate of a track being replaced, one was fully empty — neither had shown up in any prior report). Confirmed after: exactly 1 physical folder, 1 Jellyfin album entity | Claude |
| 2026-09-07 | — | Applied the same evidence-based approach to the remaining `ambiguous_for_reference` queue. 2 of 6 were **not real duplicates** and left untouched: Anthony Robbins (all 8 tracks mismatched by 2.5–10 min — different program segments, not copies) and Paul Simon *Still Crazy After All These Years* (already documented as different masters needing spectral analysis — not overridden) | Carrie Underwood *Blown Away*: deleted M4A copy, kept FLAC `(2012)`. Whitney Houston: deleted `Compilations/` copy (same format, kept copy was 2-4%/track larger + correctly organized). Paul Simon *Graceland*: deleted the 10-track base folder — fully redundant subset (including 3 vinyl-side-tagged FLAC rips filling gaps in its numbered files) of the `(1986)` 17-track FLAC deluxe edition. James Blunt *Back to Bedlam*: **not a duplicate** — the base folder held 6 genuinely unique bonus tracks (covers, live cuts) the `(2004)` FLAC edition lacked entirely; gap-filled by moving them in rather than deleting. That move created a track-number collision (moved tracks numbered 11-17 by their original tags, colliding with the destination's existing 11-12 acoustic bonus tracks) — fixed by patching the ID3 `TRCK` frames and the MP4 `trkn` atom in place (verified same-length replacement, re-read after writing, durations re-verified to confirm audio data untouched) and renaming files to match. Final state confirmed clean: 0 scatter candidates, 0 duplicates queued, 2 correctly-still-ambiguous (the two left alone) | Claude |
