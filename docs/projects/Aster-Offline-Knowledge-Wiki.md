...
| 2026-09-10 | 2 split-DNS checkpoint | After authorization, retained restrictive copies of each Pi-hole `pihole.toml` and added only `192.168.50.23 wiki.elliottrook.com` to the existing `dns.hosts` arrays. Both Pi-hole resolvers independently returned the intended address, and the normal Mac resolver path reached the certificate-valid HTTP/2 Authentik redirect. Restarting OPNsense dnsmasq cleared its cache but its independent resolver still returned no answer, demonstrating that it does not inherit this local record from the Pi-holes | Client split DNS is working through both redundant Pi-holes. The documented three-resolver gate remains open pending a matching OPNsense dnsmasq host override |
