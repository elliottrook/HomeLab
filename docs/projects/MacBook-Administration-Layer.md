...
- [ ] Generate unique protected MacBook key and record public fingerprint.
  Key generated by Jason interactively on the MacBook
  (`~/.ssh/id_ed25519_macbook_admin`, real passphrase, never scripted or seen by
  any session). Fingerprint recording is the MacBook session's own task: not yet
  synced to this doc as of this entry — see that session's evidence log.
