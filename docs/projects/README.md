...
| News aggregator (MuckScraper) | Active — Stream A; Milestone 1 complete (VLAN 70, broad egress kept, clustering method, feed list, hybrid bias methodology all decided). Milestone 2 (ingestion pipeline) next | [News aggregator (MuckScraper)](News-Aggregator-MuckScraper.md) | Private RSS/Atom reader that clusters same-story coverage across outlets, attaches an automated (non-authoritative) bias/slant estimate anchored to a named external rating source, and summarizes via the existing `aster-llama` endpoint |
