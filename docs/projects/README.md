...
| FreeCAD MCP connector | Proposed — Stream M requested, awaiting authorization | [FreeCAD MCP connector](FreeCAD-MCP-Connector.md) | Local, localhost-only MCP bridge (`neka-nat/freecad-mcp`) to let Claude drive FreeCAD directly for CAD work, starting with adapting the TrueNAS DIY SAS Expansion enclosure to fit a standard ATX PSU instead of SFX |
