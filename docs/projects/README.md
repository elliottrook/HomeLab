...
| Aster ARR stack manager | Active — advisory foundation | [Aster ARR stack manager](Aster-Arr-Stack-Manager.md) | Builds a source-aware Sonarr/Radarr/Lidarr advisor with no direct ARR credentials or action authority |
| Aster Home Assistant specialist | Active — Milestone 0 substantially complete; Milestone 1/2 groundwork built and locally tested, live deployment pending lab network access | [Aster Home Assistant specialist](Aster-Home-Assistant-Specialist.md) | Second Aster specialist domain, following the ARR project's exact milestone ladder: a source-aware advisor for the Hue/Lutron/Aqara/HomeKit Bridge deployment with no Home Assistant token or action authority yet |
...
|---|---|---|---|
| NetBox DCIM / rack & asset management | 2026-09-10 | [NetBox DCIM](completed%20projects/NetBox-DCIM.md) | Rack walk-through, VLAN/IPAM, and full guest/device inventory complete; rack diagram, `devices.conf` and IP-addressing docs reconciled against NetBox as the single source of truth; backup/recovery documented. The one previously blocked item (a live Backup Synology DSM task edit) became moot — that unit's pull role was fully replaced by the TrueNAS backup hub, which already covers the guest that item existed to add |
| NUT/UPS deployment | 2026-09-10 | [NUT/UPS handover](completed%20projects/UPS-Power-Resilience-Claude-Handover.md), [close-out report](completed%20projects/UPS-Power-Resilience-Implementation-Close-Out.md) | Three CyberPower UPS units coordinated by a bare-metal NUT server, with topology-derived low-battery thresholds and automated Proxmox/TrueNAS/Synology shutdown ordering. The unusable fourth unit (APC BN1500M2-CA, no NUT interface) is retired. `proxmox-ups` re-measured after Proxmox's full CPU/GPU/RAM upgrade landed, staying well inside its threshold margin. Controlled failure/recovery testing and push-style alerting remain intentionally deferred per Jason's decision, not gaps in the completion gate |
