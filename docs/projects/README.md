...
| AI Privileged Access Management (AI-PAM) / credential broker | Proposed — Stream M | [AI-PAM and credential broker](homelab-credential-broker.md) | OpenBao-backed central secret custody + Authentik/passkey approval + capability/MCP broker + management GUI; per-AI identities, emergency revocation, probationary onboarding, Forgejo MCP access and an AI Integration Gate for new services |
| Aster Companion App | Proposed — Stream A; Milestone 1 (discovery/risk acceptance) in progress | [Aster Companion App](Aster-Companion-App.md) | Native macOS client for Aster: Authentik passkey-only login via a new native-OIDC flow, works identically local/remote through a new NPM-proxied endpoint (no new Tailscale route), multi-persona "Aster Agents" with per-chat tool selection, a generalized gated-action framework surfacing the existing ARR-repair action, lab-hosted speech-to-text/text-to-speech, and an orb/EQ-style thinking/acting indicator; records an explicit, bounded philosophy shift toward a more autonomous Aster and excludes web-access-for-research as separate future work |
