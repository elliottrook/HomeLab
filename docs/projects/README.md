...
| News aggregator (MuckScraper) | Active — Stream A; Milestones 1-3 complete, NetBox recorded (VM id 15). Hourly pipeline live on LXC 114, VLAN 70, now ingesting 10 feeds (Al Jazeera added as a real new-source test). Outlet bias-rating values are a draft proposal awaiting Jason's confirmation before Milestone 4 finishes | [News aggregator (MuckScraper)](News-Aggregator-MuckScraper.md) | Private RSS/Atom reader that clusters same-story coverage across outlets, attaches an automated (non-authoritative) bias/slant estimate anchored to a named external rating source, and summarizes via the existing `aster-llama` endpoint |
