...
| AI Privileged Access Management (AI-PAM) / credential broker | Active — Stream A; M0–M3 complete, M4 management GUI deployed with human acceptance pending | [AI-PAM and credential broker](homelab-credential-broker.md) | Recoverable OpenBao, Unix-only capability/approval services, mobile approvals and a synthetic management view are live; lifecycle, metadata, history, audit and revocation are implemented with fresh-passkey mutations and no production credential connected |
