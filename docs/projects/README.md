...
| Infrastructure resilience and operations hardening | Proposed — decisions D1–D6 pending | [Infrastructure resilience and operations hardening](Infrastructure-Resilience-Hardening.md) | From the 2026-09-23 health check: Proxmox Backup Server with verified restores, a tested "Proxmox down" path (optional standby node), scheduled jobs moved off the Mac, pinned images with an update notifier, Doctor drift checks, failure push alerts, host swappiness and CI lint |
