...
| News aggregator (MuckScraper) | Active — Stream A; Milestones 1-2 complete, NetBox recorded (VM id 15). LXC 114 live on VLAN 70 (`192.168.70.13`), ingesting 9 feeds hourly into SQLite. Milestone 3 (clustering, bias labeling, summarization) next | [News aggregator (MuckScraper)](News-Aggregator-MuckScraper.md) | Private RSS/Atom reader that clusters same-story coverage across outlets, attaches an automated (non-authoritative) bias/slant estimate anchored to a named external rating source, and summarizes via the existing `aster-llama` endpoint |
