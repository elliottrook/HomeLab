...
| News aggregator Phase 3 — audio digest | Active — Stream A; Milestones 1-3 complete, pending Jason's final mobile-rendering confirmation | [News aggregator Phase 3](News-Aggregator-Audio-Digest.md) | Local Piper TTS narrates the latest digest run (headline + abridged summary per story) as a single MP3 briefing on `/digest`, chained into the existing twice-daily timer; also closes a real HomeLab Doctor gap (a silently-stale audio file) and gives Jason a real trial of Piper ahead of a planned Home Assistant project |
