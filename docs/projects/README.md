...
| AI Privileged Access Management (AI-PAM) / credential broker | Active — Stream A; M0–M8 complete, M9 graduation active | [AI-PAM and credential broker](homelab-credential-broker.md) | Recoverable OpenBao, lifecycle/approval management, bounded Forgejo read/write and Doctor paths, native macOS parity, authoritative documentation and read-only policy-drift monitoring are complete; outage/restart/restore graduation remains |
