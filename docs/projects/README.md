...
| AI Privileged Access Management (AI-PAM) / credential broker | Active — Stream A; M0–M3 complete, M4 management GUI next | [AI-PAM and credential broker](homelab-credential-broker.md) | Recoverable OpenBao foundation, Unix-only capability broker and Aster Companion approval inbox are live with synthetic data only; real fresh-passkey Red approval, Yellow denial, exact consumption, replay/change/timeout failures and zero-open-request closeout passed |
