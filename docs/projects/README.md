...
| Document OCR + summarization | Proposed | [Document OCR + summarization](Document-OCR-Summarization.md) | Summarization-only layer over Paperless-ngx's own OCR (Paperless itself is not yet deployed — treated as a real prerequisite); open decision is one combined project versus splitting infra deployment from the AI layer |
