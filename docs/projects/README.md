...
| News aggregator (MuckScraper) | Active — Stream A; Milestones 1-4 complete, NetBox recorded (VM id 15). Hourly pipeline and reading UI live on LXC 114, VLAN 70 (`news.internal`, 10 feeds). Outlet bias-rating values are a draft proposal awaiting Jason's confirmation before Milestone 5 | [News aggregator (MuckScraper)](News-Aggregator-MuckScraper.md) | Private RSS/Atom reader that clusters same-story coverage across outlets, attaches an automated (non-authoritative) bias/slant estimate anchored to a named external rating source, and summarizes via the existing `aster-llama` endpoint |
