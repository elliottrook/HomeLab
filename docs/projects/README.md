...
| News aggregator Phase 2 — digest and sections | Active — Stream A; Milestone 1 starting | [News aggregator Phase 2](News-Aggregator-Digest-and-Sections.md) | Twice-daily abridged/deviation-note digest layered on Phase 1's hourly breaking-news pipeline via the existing `aster-llama` key, a redesigned card UI for the digest view only, and a settings page to queue new source/section requests for verification — built on the graduated [Phase 1](completed%20projects/News-Aggregator-MuckScraper.md) |
