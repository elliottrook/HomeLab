...
| News aggregator (MuckScraper) | Proposed — no stream selected, several open decisions require Jason's input before Milestone 1 can complete | [News aggregator (MuckScraper)](News-Aggregator-MuckScraper.md) | Private RSS/Atom reader that clusters same-story coverage across outlets, attaches an automated (non-authoritative) bias/slant estimate, and summarizes via the existing `aster-llama` endpoint. Reconciled 2026-09-14 from the `github` remote, where it had been committed directly instead of through Forgejo `origin` |
