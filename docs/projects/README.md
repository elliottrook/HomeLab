...
| AI Privileged Access Management (AI-PAM) / credential broker | Active — Stream A; M0–M4 complete, M5 probationary lifecycle next | [AI-PAM and credential broker](homelab-credential-broker.md) | Recoverable OpenBao, Unix-only capability/approval services, mobile approvals and management are live with synthetic data only; iPhone lifecycle, audit and agent/service/global revocation passed with fresh passkeys and zero open requests |
