...
| Aster mirror directory-first retrieval | Proposed — Stream A requested, awaiting authorization | [Aster mirror directory-first retrieval](Aster-Mirror-Directory-Retrieval.md) | Measures whether flat per-claim retrieval in the graduated 30-source/1,796-entry mirror has degraded at production scale, and only if justified, adds a deterministic per-source directory/abstract layer ahead of entry ranking, gated behind a proven fallback to today's flat search |
