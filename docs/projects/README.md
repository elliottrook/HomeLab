...
| AI Privileged Access Management (AI-PAM) / credential broker | Active — Stream A; M0–M5 complete, M6 Forgejo MCP pilot next | [AI-PAM and credential broker](homelab-credential-broker.md) | Recoverable OpenBao, Unix-only capability/approval services, mobile approvals/management and mandatory Probation are live with synthetic data only; replacement promotion/retirement and adversarial scope/revocation passed with zero open requests |
