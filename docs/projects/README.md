...
| Video library archiving | Proposed — Milestone 1 (discovery/dry-run) complete | [Video library archiving](Video-Library-Archiving.md) | Downconverts aged current-library video and hands it to the archive roots above. Dry run validated against real data; production age threshold currently finds 0 candidates because the Plex-to-Jellyfin migration reset Radarr/Sonarr's `dateAdded` library-wide — next candidates expected ~Feb-March 2027 |
