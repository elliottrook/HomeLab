...
| Aster mirror directory-first retrieval | Active — Stream A; Milestones 1-2 complete, Milestone 3 and Milestone 4's retrieval comparison complete, live adversarial/regression suite re-run outstanding | [Aster mirror directory-first retrieval](Aster-Mirror-Directory-Retrieval.md) | Measures whether flat per-claim retrieval in the graduated 30-source/1,796-entry mirror has degraded at production scale; built and validated a directory-first narrowing layer (8/10 vs. 6/10 correct on the real corpus, zero regressions), gated behind a proven fallback, not yet activated in production |
...
| News aggregator (MuckScraper) | Active — Stream A; Milestones 1-4 complete, NetBox recorded (VM id 15). Hourly pipeline and reading UI live on LXC 114, VLAN 70 (`news.internal`, 10 feeds — AP News dropped, no accessible source-published feed). Outlet bias ratings (BBC, NPR; cited to AllSides) loaded and rendering live with an unverified-this-session tooltip; Milestone 5 not started | [News aggregator (MuckScraper)](News-Aggregator-MuckScraper.md) | Private RSS/Atom reader that clusters same-story coverage across outlets, attaches an automated (non-authoritative) bias/slant estimate anchored to a named external rating source, and summarizes via the existing `aster-llama` endpoint |
