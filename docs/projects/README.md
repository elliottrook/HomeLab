...
| AI Privileged Access Management (AI-PAM) / credential broker | Active — Stream A; M0–M2 complete, M3 deployed with real-device passkey acceptance pending | [AI-PAM and credential broker](homelab-credential-broker.md) | Recoverable OpenBao foundation, Unix-only capability broker and Aster Companion approval inbox are live with synthetic data only; changed-payload, replay and timeout failures pass, while real iPhone approve/deny remains the M3 gate |
