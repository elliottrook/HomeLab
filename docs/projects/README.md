...
| News aggregator (MuckScraper) | Active — Stream A; Milestone 1 (discovery and design decisions) starting. Outbound-firewall and bias-methodology decisions remain explicit Jason checkpoints regardless of stream | [News aggregator (MuckScraper)](News-Aggregator-MuckScraper.md) | Private RSS/Atom reader that clusters same-story coverage across outlets, attaches an automated (non-authoritative) bias/slant estimate, and summarizes via the existing `aster-llama` endpoint. Reconciled 2026-09-14 from the `github` remote, where it had been committed directly instead of through Forgejo `origin` |
