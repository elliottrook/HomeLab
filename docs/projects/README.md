...
| Video library archiving | Proposed — Milestone 2 substantially complete for TV; movie side untested | [Video library archiving](Video-Library-Archiving.md) | Downconverts aged current-library video and hands it to the archive roots above. GPU-accelerated (Intel Arc A380, discovered mid-project) via the Jellyfin container; a full 8-episode season archived successfully after four real bugs were found and fixed. Age threshold revised to ~4 months, anchored per-season for TV. Production threshold currently finds 0 further candidates until the Plex-to-Jellyfin migration's `dateAdded` reset ages out (~Feb-March 2027) |
...
| Backup architecture redesign | Active — Milestone 4 passed; Milestone 5 (cutover) in progress | [Backup architecture redesign](Backup-Architecture-Redesign.md) | Replaces Hyper Backup's Synology-to-Synology path with Synology → rsync → TrueNAS/ZFS snapshots → dedicated rclone LXC → encrypted, versioned IDrive e2. All three legacy Hyper Backup jobs are stopped; documentation/inventory cleanup remains |
| Media archive backup to Synology | Proposed — scoped, not started | [Media archive backup to Synology](Media-Archive-Synology-Backup.md) | Backs up TrueNAS's `archive-movies`/`archive-tv` (5.6 TB) to `gowest` using the same plain-rsync approach as the backup redesign — but in the reverse direction, which needs Jason's explicit sign-off before any build, since it touches that project's "gowest is a source, never a destination" decision |
