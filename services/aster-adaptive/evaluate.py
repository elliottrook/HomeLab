...
def run(repeats=100):
    ...
    for f in fixtures:
        ...
        for _ in range(repeats):
            ...
            start=time.perf_counter_ns(); decision=selector.propose(req,registry,messages,allowed=allowed,now=1); validate_proposal(req,decision,registry,now=1); adapter_times.append(time.perf_counter_ns()-start)
        ...
        outcome=Outcome(run_id=f['id'],result='selection_recorded' if names else 'abstained',label_source='synthetic_expectation',route_correct=None)
        outputs.append({'fixture_id':f['id'],'selected':names,'decision':decision.model_dump(),'run':run_record.model_dump(),'outcome':outcome.model_dump()})
    def timing(values):
        ordered=sorted(values)
        return {'p50_ns':int(statistics.median(values)),'p95_ns':ordered[int(.95*(len(ordered)-1))]}
    manifest={str(p.relative_to(ROOT)):hashlib.sha256(p.read_bytes()).hexdigest() for p in [SOURCE,*sorted(HERE.glob('*.py')),HERE/'fixtures.json',*sorted((ROOT/'schemas/aster').glob('*.json'))]}
