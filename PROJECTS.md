...
|---|---|---|
| 2026-09-15 | Created 11 new Proposed-stage project charters for the remaining un-started ideas from Jason's local-AI brainstorm list (Home Assistant voice assistant, Recommendarr watch recommendations, email triage digest, calendar personal assistant, combined morning digest, document OCR + summarization, local subtitle generation/translation, video duplicate/quality-upgrade finder, auto-written Jellyfin collection descriptions, music recommender, book recommender), each following `docs/Project-Creation-Standard.md`'s template with an explicit pre-start risk assessment, open architectural decisions and unchecked milestones. None are authorized for implementation. Added to the portfolio table. | `docs/projects/README.md` portfolio table; individual project documents under `docs/projects/` |
