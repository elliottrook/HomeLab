...
### Added
- Staged Aster's single-operation ARR repair boundary in production and exercised its first live eligibility gate. The broker ran under a dedicated unprivileged account with execution disabled, accepted only Aster's exact host through a temporary host/port firewall rule, and kept the Radarr credential on TrueNAS. The one authorized read-only scan found zero eligible stale completed records, so it issued no candidate, created no approval or audit attempt, and made no Radarr change. The sanitized empty state was pushed to Aster, the broker was stopped and left boot-disabled, and the temporary firewall rule was removed. Final disposable-runtime regressions passed 61/61 broker and 42/42 Aster tests.
