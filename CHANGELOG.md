...
### Changed
- Activated Aster's directory-first retrieval on deterministic mirror pipeline 1.5.0 after credential rotation, retained rollback, zero-drift corpus verification and accepted 60/60 production behavior accounting. Graduation remains held on a newly isolated infrastructure defect: the 02:30 all-guests backup starts stopped rollback VM 105, binds its persistent B60 passthrough device to `vfio-pci`, and fails to return it to the host `xe` driver when backup QEMU exits.
