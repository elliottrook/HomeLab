...
### Added
- Graduated Aster's read-only Forgejo and NetBox integration without placing source credentials or a new network path in Aster. Dedicated least-privilege identities query only source-local loopback APIs; strict producers exclude repository content, messages, authors, workflow logs, NetBox config contexts, custom fields, contacts and secrets; Proxmox validates and transports only root-owned sanitized JSON every five minutes; and Aster mounts the reports read-only. Local report tests passed 13/13, deployed Aster tests passed 40/40, live model cases passed 4/4, and deliberately invalid write probes were denied with HTTP 403 and unchanged object counts on both services.
