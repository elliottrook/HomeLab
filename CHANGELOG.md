...
### Added
- Started the Aster Home Assistant Specialist project, Aster's second specialist domain after the ARR stack manager, following the exact same milestone ladder and testing rigor. Built and locally tested (66 tests, zero regressions in the existing suite): a strict aggregate-only sanitized report contract (`services/aster-agent/home_assistant_report.py`, per-domain entity counts only, no entity_id/name/room/automation-name), a report producer against Home Assistant's REST API with private-token handling and a documented per-domain state classification (`scripts/produce-aster-homeassistant-report.py`), a CLI validator, a read-only `get_home_assistant_report` tool wired into the Aster agent with an advisory safety boundary, and a versioned Hermes `home-assistant-manager` skill. Live deployment (Hermes/LXC 104 installation, a real Home Assistant token, and agent-level live testing) is pending a session with actual lab network access, which this cloud session does not have. Recorded a per-project authorization and a 4-hourly self-resuming Routine so the project continues autonomously through Aster's graduation gate for this specialty.
- Closed both residual Phase 1 PoE-switch follow-ups: Jason removed the retired UniFi PoE switch's stale entry from the UniFi Network app, and the Mac-only Arista management access failure from the 2026-08-23 incident was retested and found not reproducible (no root cause identified, so it should be reopened if it recurs rather than assumed transient again). The Phase 1 reliability close-out has no remaining open items.
- Closed out the NUT/UPS power-resilience project now that its last two open items are resolved (APC BN1500M2-CA retired, `proxmox-ups` re-measured post-upgrade — see below). Moved the handover and implementation close-out documents into `docs/projects/completed projects/`, joining the standard tracked-project structure, and updated every cross-reference across the repo (roadmap, portfolio index, architecture, hardware inventory, operations, network baseline, `PROJECTS.md`, `CLAUDE.md`) to the new path.
- Retired the old APC Back-UPS Pro BN1500M2-CA (never NUT-compatible, carried no equipment) and closed the `proxmox-ups` hardware-upgrade re-measurement follow-up: with Proxmox's CPU, GPU and RAM upgrades all now landed, live `upsc` telemetry shows `ups.load` 14% (~140W), `battery.charge` 100%, `battery.runtime` ~3350s (~55.8 min) — essentially flat versus the RAM-only baseline and comfortably inside the 80% low-battery threshold's margin, so no Milestone 3 threshold changes are needed. Updated the power topology table, architecture diagram and hardware inventory accordingly.
- Closed out the NetBox DCIM / rack & asset management enhancement project: every milestone (physical inventory, deployment, data population, cutover/hand-back) is complete, the rack diagram and `devices.conf`/IP-addressing docs are reconciled against NetBox as the single source of truth, and backup/recovery is documented. Moved the project document into `docs/projects/completed projects/` and updated the portfolio and roadmap accordingly.
