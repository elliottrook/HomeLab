...
- Graduated the Local AI enhancement project after reconciling its original
  ...
  running guests, B60 `xe`/Vulkan health, stopped VM 105 rollback, current guest
  archives and off-host mirror, production snapshots, Beszel/Doctor coverage,
  and the already-proven encrypted recovery chain. Removed the exact inactive
  Hermes `auth.json` after a redacted audit found it was the only runtime file
  carrying populated `openai-codex` access/refresh-token fields; a post-delete
  scan found none remaining and Aster/llama.cpp stayed active. Activated a clean
  26-source snapshot from closeout commit `c31bf5e`; its provenance, permissions,
  Aster-account retrieval and 55/55 deployed tests passed, with the previous
  `a9d4a5c` deployment retained as rollback. After current `main` was merged,
  activated a second deterministic 26-source snapshot from commit `7658c9a`;
  provenance, permissions, Aster-account retrieval and 55/55 deployed tests
  passed again, with `c31bf5e` retained as the new rollback point.
- Graduated Aster's read-only Forgejo and NetBox integration without placing source credentials or a new network path in Aster. Dedicated least-privilege identities query only source-local loopback APIs; strict producers exclude repository content, messages, authors, workflow logs, NetBox config contexts, custom fields, contacts and secrets; Proxmox validates and transports only root-owned sanitized JSON every five minutes; and Aster mounts the reports read-only. Local report tests passed 13/13, deployed Aster tests passed 41/41, live model cases passed 4/4, and deliberately invalid write probes were denied with HTTP 403 and unchanged object counts on both services.
- Graduated the full Aster ARR stack-manager curriculum. Added the reviewed, provenance-bearing six-service operational reference; inventoried versions, roles, ports, roots, downloader relationships, current sanitized health and all known mutation-capable automations; taught Aster to retrieve long inventory/automation sections across chunk boundaries; and added focused ARR curriculum evaluations with summary-only output. Final gates passed 61/61 broker tests, 43/43 disposable Aster tests, 42/42 deployed Aster tests, focused curriculum 4/4, current ARR advisory 8/8, bounded report 1/1, stale-config 1/1, legacy ARR 6/6 and original Aster graduation 14/14. Production returned to least privilege: the broker is stopped and boot-disabled, port 9421 has no listener or firewall rule, candidates and approvals are empty, and every future live attempt still needs a fresh action-specific approval.
- Graduated Aster's first bounded ARR repair through the production-shaped deployment without manufacturing a fault in live Radarr. After the live eligibility gate correctly refused zero matching records, a disposable TrueNAS-local Radarr endpoint supplied one synthetic stale completed queue record to the deployed broker and Aster structured endpoint. The approved execution returned `dismissed`; the target observed exactly GET, fixed safe DELETE and verification GET, and replay was denied with no additional target call. The original broker environment/state and empty Aster report were restored, the broker was stopped and left boot-disabled, and the exact temporary firewall rule was removed. Live Radarr was never targeted or changed. Final regressions passed 61/61 broker and 42/42 Aster tests.
- Reorganized HomeLab Doctor's output into accurately-named categories (Network & Connectivity, Core Infrastructure, Applications & Services, Automation Pipelines, Service Reachability, Backups, Local Environment), replacing a stale grouping where "Checking internet connectivity" had quietly grown to include Arista, Proxmox, TrueNAS, Frigate, UniFi, NetBox, Observability, Aster and NUT as the script grew over many sessions. Added a "Needs Attention" recap that reprints every warning/failure by name right before the summary, so a run with 60+ passing checks doesn't require scrolling to find the 2-3 real issues. Purely a display reorganization — every individual check function is unchanged, verified by an identical pass/warn/fail count before and after. Labeled "HomeLab Doctor v2" in its own header; the toolkit-wide `VERSION` file is untouched, since that tracks actual infrastructure-completion milestones, not script formatting.
- Migrated 95 GB of photos (2,901 files) from the Mac to the Synology after Synology Drive proved unable to move them, verifying the copy by exact file count, exact total bytes (101,797,761,446) and SHA-256 on a sample including the three largest files. DSM refuses `rsync --server` for non-root users, so the transfer used chunked `tar` over SSH with a resumable inventory-diff wrapper.
- Verified backup integrity against the newly found network fault rather than trusting reported status: `zstd -t` passed on the three most recent Proxmox archives plus the 9 GB `qemu-105` archive, and five archives matched their source byte-for-byte. Backups were never affected — they cross Management→Servers, not the faulty host pair. The empty `mac` backup set is expected, as the redesign records that leg as not yet started.
...
- Added `check_wireless_tagging` to HomeLab Doctor, a config-drift check guarding the newly explicit SSID tagging. It asserts every enabled SSID is bound to a VLAN-carrying network and that `GoWest`/`TELUS96FF`/`Anchors_Rest` sit on VLANs 10/30/40. This catches a regression no health check can: an SSID rebound to an untagged network keeps working perfectly, because Arista Et33's native VLAN is still 10, silently restoring the hidden dependency the tagging change removed. Unrecognised SSIDs warn rather than fail, so legitimate additions don't break the run. Trusted VLAN 10 deliberately gets no lease-count check, since VLAN 10 also carries wired clients and would only sag rather than collapse; that failure mode is already covered by `check_unifi`. Detection logic was verified against stubbed API responses covering correct config, an untagged revert, a wrong VLAN, a missing SSID and an unexpected new SSID.
...
### Changed
- Graduated Aster's directory-first retrieval on deterministic mirror pipeline 1.5.0 after credential rotation, retained rollback, zero-drift corpus verification and accepted 60/60 production behavior accounting. Root-caused recurring B60 loss to the 02:30 all-guests backup starting stopped rollback VM 105 with a persistent GPU mapping; removed that mapping and proved the real backup path with 88/88 one-second samples remaining on `xe`, then passed authenticated inference and strengthened Doctor validation. VM 105 remains a recoverable disk/config rollback with on-demand-only B60 attachment.
- Raised NetBox LXC 111 from 2048 to 4096 MB via live `pct set`, applied without a restart so the containers kept their existing uptime. This was headroom rather than a fix: the stack sat at ~1.4 GB of its 2 GB cap with ~600 MB free and the Django container alone holding ~1 GB, but no OOM events appeared in syslog or dmesg and swap was untouched — bulk imports and concurrent API use were the realistic pressure. Afterwards 2.6 GB was available with all five containers healthy, the API returning 200 and Doctor's NetBox check green. The new figure matches LXC 100/101/106 and moves host allocation from ~46 GB to ~48 GB of 78 GB.
- Accepted a new configuration-drift baseline (2026-08-29 exports → 2026-09-02 exports, 70 protected entries) after auditing all twelve reported differences against the work that produced them. Every change was additive and attributable — eight narrowly scoped OPNsense firewall rules for backup pulls, observability scraping and Forgejo/Grafana proxying with Authentik OIDC; two DNS host overrides; three new Proxmox guests (109/110/111); rollback snapshots on 104 and 105 plus GPU passthrough; and read-only PVEAuditor accounts for Prometheus and the Homepage widget. Nothing was removed, broadened or weakened anywhere, and the Arista change (Et17 Apple TV VLAN move) was present in both running and startup config. The audit method and a full per-entry table are recorded in `docs/04-Operations.md` so the next alert has a reference point.
- Eliminated the last untagged wireless path by creating a `Trusted` VLAN 10 network in UniFi and rebinding the `GoWest` SSID to it. All three SSIDs are now explicitly tagged (10/30/40), so no wireless traffic depends on Arista Et33's native VLAN. Previously nothing in UniFi stated that Trusted was VLAN 10 — `GoWest` sat on the `Default` network with no VLAN ID and became VLAN 10 only through a native-VLAN setting two devices away, meaning an innocuous-looking edit to Et33 would have silently moved the entire household WLAN onto another VLAN. The change was deliberately traffic-equivalent (Et33's native VLAN was already 10, and both trunks already permitted tagged VLAN 10), so no switch changes were required; verified afterwards with 55 clients associated, 17 Trusted leases, 18 MACs on VLAN 10 via Et33 and both APs online. Changing Et33's native VLAN was deliberately not done, since its only remaining benefit would be relocating the AP Switch's untagged management plane, which now has an honest documented address.
- Reissued `/root/.netbox-api-token` on LXC 111 as a read-only token after finding it rejected with `403 Invalid v1 token`. The file held a legacy 40-character v1-format value matching no token in the database; NetBox 4.6.9 issues v2 tokens stored hashed as a 12-character key plus an HMAC digest over a server-side pepper, with the secret recoverable only at creation and presented as `Authorization: Bearer nbt_<key>.<secret>`. The replacement is deliberately `write_enabled=False` since its consumers are health checks and inventory reads, and was verified 200 on a device read and 403 on an attempted write. Two undescribed install-time admin tokens remain and are logged as an open hygiene item rather than pruned, since an out-of-repo consumer cannot be ruled out.
...
### Fixed
- Root-caused and fixed a traffic fault where sustained transfers from Trusted VLAN 10 to the main Synology at `192.168.20.41` failed while small ones succeeded. The NAS was dual-homed (`eth0` on VLAN 20, `eth1` as `192.168.1.41` on VLAN 10), giving it a directly-connected route back to VLAN 10: requests routed in through OPNsense but replies left via `eth1` directly, so OPNsense saw only one direction and its state tracking desynced — 503,278 state-mismatches and climbing, with paired states disagreeing. Short flows finished before pf gave up; sustained flows died mid-stream. Isolated by testing 200 MB transfers to five destinations, which showed other VLAN 20 hosts were fine and the fault was specific to the only dual-homed host, with both its NICs error-free. Fixed by bonding `eth0`+`eth1` in active-backup mode onto a single `192.168.20.41` address (matching TrueNAS's existing pattern) and moving Arista `Et24` to access VLAN 20. Verified: a 1 GB sustained transfer now succeeds, and Synology Drive committed 1.48 GB in 45 seconds after 17 hours of zero progress with its connection resets stopping entirely.
- Recorded that the bond must be created *before* the switch-port VLAN change; doing it the other way round breaks NAS access for every Trusted device in the interim.
- Reverted the 2026-09-04 GoWest VLAN 10 tagging change, which broke the main WLAN. Arista Et33's native VLAN is 10, so while tagged and untagged frames arrive identically on ingress, Arista returns VLAN 10 traffic *untagged* on egress — and an SSID bound to a tagged VLAN 10 network drops it. Return traffic including DHCP OFFER/ACK was lost at the AP. The fault was masked for hours because clients holding pre-change 24-hour leases stayed associated and looked healthy, so the original post-change verification (counting clients and leases) proved nothing; symptoms only appeared as clients renewed or re-associated, giving "connected, no internet" for those with live leases and 169.254.x self-assignment for new ones. IoT and Guest were unaffected throughout, since VLANs 30 and 40 are not the native VLAN. After the revert all 8 GoWest clients recovered valid addresses, including one stuck on APIPA for over 16 hours. `check_wireless_tagging` was rewritten to fail if GoWest is ever re-tagged, and the baseline records that Et33's native VLAN must move off 10 before any retry.
